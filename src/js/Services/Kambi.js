...
const getTournamentData = (filter, criterionIds, dateRanges) => {
  ...
  return Promise.all(tournamentRequests)
    .then(tournamentData => {
      ...
      const now = new Date()
      finalType =
        now < new Date(dateRanges.quarter.end)
          ? 'quarter'
          : now < new Date(dateRanges.semi.end)
            ? 'semi'
    ...
    .then(([eventsWithInitialOffers, ...finalEvents]) => {
      ...
      const eventsByStart = eventsWithAllBetOffers.sort(
...
//         event.toQualifyBetOffer = event.betOffers.find(
//           offer => data.qualifyCriterionId.indexOf(offer.criterion.id) >= 0
//         )
...
//             const rematch = events.find(
//               ev =>
//                 event.event.homeName === ev.event.awayName &&
//                 event.event.awayName === ev.event.homeName
//             )
...
//               const tempEnglishLabel =
//                 tournamentBetOffer.outcomes[iRev].englishLabel
//               const tempType = tournamentBetOffer.outcomes[iRev].type

//               tournamentBetOffer.outcomes[iRev].label =
//                 tournamentBetOffer.outcomes[i].label
//               tournamentBetOffer.outcomes[iRev].englishLabel =
//                 tournamentBetOffer.outcomes[i].englishLabel
//               tournamentBetOffer.outcomes[iRev].type =
//                 tournamentBetOffer.outcomes[i].type
